...
public class HBZTabCompleter implements TabCompleter {
      ...
      if (args.length == 2) {
         ...
         if (args.length == 4) {
            ...
            return !subCommandx.equals("area") && !subCommandx.equals("a")
               ...
               : this.filterCompletions(Arrays.asList("all", "hostile", "peaceful", "items", "animals", "mobs"), args[3]);
   ...
   private List<String> handleVillagerCompletion(String[] args) {
      if (args.length == 2) {
         ...
         return (List<String>)(args.length == 3 && args[1].equalsIgnoreCase("optimize") ? this.getWorldNames(args[2]) : new ArrayList<>());
      }
   }

   private List<String> getWorldNames(String partial) {
      List<String> worldNames = Bukkit.getWorlds().stream().<String>map(WorldInfo::getName).collect(Collectors.toList());
      return this.filterCompletions(worldNames, partial);
   }

   private List<String> getPlayerNames(String partial) {
      List<String> playerNames = Bukkit.getOnlinePlayers().stream().<String>map(Player::getName).collect(Collectors.toList());
