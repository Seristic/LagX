...
public class LRConfig {
    ...
    public static void reload() {
        ...
        try {
            configuration.load(new File(LaggRemover.getInstance().getDataFolder(), "config.yml"));
        } catch (IOException | InvalidConfigurationException e) {
            LaggRemover.getInstance().getLogger().severe("Error loading configuration:");
            e.printStackTrace();
            configuration = LaggRemover.getInstance().getConfig();
        }
        ...
        if (!noSaveWorlds.contains("DISABLED")) {
            for (World w : Bukkit.getWorlds()) {
                if (noSaveWorlds.contains(w.getName())) {
                    w.setAutoSave(false);
                    LaggRemover.getInstance().getLogger().info("World \"" + w.getName() + "\" will not automatically save.");
                }
            }
        }
    }
    ...
    private static DoubleVar<Object[], Boolean> loadP(LRProtocol p, String lpk, FileConfiguration f) {
        try {
            ...
            LaggRemover.getInstance().getLogger().info("Error parsing protocol info for \"" + lpk + "\": " + e.toString());
            return null;
        }
    }

    public static void init() {
        File config = new File(LaggRemover.getInstance().getDataFolder(), "config.yml");
        if (!config.exists()) {
            LaggRemover.getInstance().saveDefaultConfig();
        }
