...
public class ItemFrameOptimizer implements Listener {
   ...
   public ItemFrameOptimizer(HBZCleaner plugin) {
      this.plugin = plugin;
      this.loadConfig();
      if (this.enabled) {
         Bukkit.getPluginManager().registerEvents(this, plugin);
         Bukkit.getAsyncScheduler().runAtFixedRate(plugin, task -> this.optimizeItemFrames(), 30L, 30L, TimeUnit.SECONDS);
                     ...
                     task -> {
                        ...
                        if (this.debugMode && itemFrameCount > 0) {
                           this.plugin
                              .getLogger()
                              .info(
                                 "Optimized " + itemFrameCount + " item frames in chunk " + chunk.getX() + "," + chunk.getZ() + " in world " + world.getName()
                              );
                        }
                     }
                  );
   ...
   public void onItemFramePlace(HangingPlaceEvent event) {
      if (this.enabled) {
         if (event.getEntity() instanceof ItemFrame && this.debugMode) {
            this.plugin.getLogger().info("Item frame placed at " + event.getEntity().getLocation() + " - will be optimized by ItemFrameOptimizer");
         }
      }
   }
