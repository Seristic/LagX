...
public class HBZConfig {
   ...
   public static void reload() {
      ...
      try {
         configuration.load(new File(HBZCleaner.getInstance().getDataFolder(), "config.yml"));
      } catch (InvalidConfigurationException | IOException var10) {
         HBZCleaner.getInstance().getLogger().severe("Error loading configuration:");
         var10.printStackTrace();
         configuration = HBZCleaner.getInstance().getConfig();
      }
      ...
      HBZCleaner.getInstance()
         .getLogger()
         .info("Config loaded - auto-lag-removal.run = " + autoLagRemoval + ", auto-lag-removal.every = " + autoLagRemovalTime);
      HBZCleaner.getInstance().getLogger().info("Automatic cleanup: " + (autoLagRemoval ? "enabled (every " + autoLagRemovalTime + " minutes)" : "disabled"));
      ...
      if (!noSaveWorlds.contains("DISABLED")) {
         for (World w : Bukkit.getWorlds()) {
            if (noSaveWorlds.contains(w.getName())) {
               w.setAutoSave(false);
               HBZCleaner.getInstance().getLogger().info("World \"" + w.getName() + "\" will not automatically save.");
            }
         }
      }
   }
   ...
   private static DoubleVar<Object[], Boolean> loadP(LRProtocol p, String lpk, FileConfiguration f) {
      try {
         ...
         HBZCleaner.getInstance().getLogger().info("Error parsing protocol info for \"" + lpk + "\": " + var4.toString());
         return null;
      }
   }

   public static void init() {
      File config = new File(HBZCleaner.getInstance().getDataFolder(), "config.yml");
      if (!config.exists()) {
         HBZCleaner.getInstance().saveDefaultConfig();
      }

      if (DrewMath.intFrom(Objects.requireNonNull(HBZCleaner.instance.getConfig().getString("version"))) < 16) {
         HBZCleaner.instance
            .getLogger()
            .info(
         ...
         try {
            FileWriter w = new FileWriter(
               new File(
                  HBZCleaner.instance.getDataFolder(),
                  "config(backup-" + new SimpleDateFormat("yyyy-MM-dd-HHmmss").format(Calendar.getInstance().getTime()) + ").yml"
               )
            );
            ...
            HBZCleaner.instance.getLogger().info("An error occurred when backing up the old configuration (" + var2.getMessage() + ").");
         }

         if (config.delete()) {
            HBZCleaner.instance.saveDefaultConfig();
         } else {
            HBZCleaner.instance
               .getLogger()
   ...
   private static void check(String version) {
      try {
         if (!Objects.equals(HBZCleaner.instance.getConfig().getString("version"), version)) {
   ...
   private static void updateConfig(String config_version) {
      ...
      FileConfiguration c = HBZCleaner.instance.getConfig();
      ...
      if (newConfig.size() != 0) {
         ...
         try {
            ...
            c.save(new File(HBZCleaner.instance.getDataFolder(), "config.yml"));
         } catch (IOException var5) {
         }
      }

      HBZCleaner.instance.getLogger().info("Your configuration file was updated to v" + config_version);
   }
   ...
   private static HashMap<String, Object> getConfigVals() {
      ...
      try {
         config.loadFromString(stringFromInputStream(HBZCleaner.class.getResourceAsStream("/config.yml")));
