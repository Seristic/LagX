...
public class Events implements Listener {
   ...
   public void onWorldLoad(WorldInitEvent e) {
      if (HBZConfig.noSpawnChunks) {
         HBZCleaner.instance.getLogger().warning("Config `noSpawnChunks` is not supported in Folia, disabled.");
      }
   }
   ...
   public void onChat(AsyncPlayerChatEvent e) {
      ...
      if (!HBZCleaner.hasPermission(p, "hbzcleaner.nochatdelay") && HBZConfig.chatDelay > 0) {
      ...
      if (HBZConfig.doRelativeAction && !useLocationLagRemoval.contains(uuid) && e.getMessage().toLowerCase().contains("lag")) {
         if (canSLDRun && HBZConfig.isAIActive) {
            this.smartLagDetection();
         }

         List<Entity> nearbyEntities = p.getNearbyEntities((double)HBZConfig.localLagRadius, (double)HBZConfig.localLagRadius, (double)HBZConfig.localLagRadius);
   ...
   private void cooldown(UUID u) {
      ...
      Bukkit.getAsyncScheduler()
         .runDelayed(
            HBZCleaner.getInstance(), task -> useLocationLagRemoval.remove(u), 20L * (long)HBZConfig.localLagRemovalCooldown * 50L, TimeUnit.MILLISECONDS
         );
   }

   private void smartAIcooldown() {
      canSLDRun = false;
      Bukkit.getAsyncScheduler().runDelayed(HBZCleaner.getInstance(), task -> canSLDRun = true, 1200L * HBZConfig.smartaicooldown * 50L, TimeUnit.MILLISECONDS);
   }

   private void chatDelayCooldown(UUID uuid) {
      chatDelay.add(uuid);
      Bukkit.getAsyncScheduler().runDelayed(HBZCleaner.getInstance(), task -> chatDelay.remove(uuid), (long)HBZConfig.chatDelay * 50L, TimeUnit.MILLISECONDS);
   }
