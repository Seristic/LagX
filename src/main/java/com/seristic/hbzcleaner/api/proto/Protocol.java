...
public class Protocol {
   ...
   public static Counter getCounter(String p) {
      ...
      List<String> args = HBZCleaner.instance.getConfig().getStringList("protocol_warnings." + p + ".stages");
      if (args == null) {
         args = Collections.emptyList();
      }

      for (String arg : args) {
         final String[] a = arg.replaceAll("&", "§").replaceAll("%PREFIX%", HBZCleaner.prefix).split(":");
      ...
      Counter counter = new Counter(HBZCleaner.instance.getConfig().getLong("protocol_warnings." + p + ".time")) {
         @Override
         public void onFinish() {
            if (var2 != null) {
               HBZCleaner.broadcastWarn(var2);
            }
         }
      };
