...
public class CCItems implements LRProtocol {
   ...
   private static final String help = new HelpFormatter()
         .set(HelpFormatter.HelpFormatterType.DESCRIPTION,
               "§eRemoves items from all worlds, selected worlds, or selected chunks.")
   ...
   private int clearItems(List<Entity> ents, boolean count) {
      ...
      try {
         deathTracker = LagX.getInstance().getPlayerDeathTracker();
      ...
      if (protectedItems > 0) {
         LagX.getInstance().getLogger()
               .info("Protected " + protectedItems + " items from auto-clear due to recent player deaths");
      }
      ...
      if (count) {
         ...
         for (World world : Bukkit.getWorlds()) {
            for (Chunk chunk : world.getLoadedChunks()) {
               ...
               Bukkit.getRegionScheduler().run(LagX.getInstance(), world, chunk.getX(), chunk.getZ(), task -> {
   ...
   private int clearItemsFromWorld(World world, boolean count) {
      ...
      if (count) {
         ...
         if (protectedItems > 0) {
            LagX.getInstance()
                  .getLogger()
                  .info("Protected " + protectedItems + " items from auto-clear due to recent player deaths in world "
                        + world.getName());
         }
