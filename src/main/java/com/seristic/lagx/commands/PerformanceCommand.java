...
public class PerformanceCommand implements CommandExecutor, TabCompleter {
   ...
   public PerformanceCommand(LagX plugin) {
      this.plugin = plugin;
   ...
   private void showSummary(CommandSender sender) {
      ...
      sender.sendMessage(
            ColorUtil.color("&b— Utilisation: &3" + String.format("%.2f", systemLoad * 100.0) + "% &b/ &3100.0%"));
      ...
      sender.sendMessage(
            ColorUtil.color("&b— Load rate: &3" + String.format("%.2f", loadRate) + "&b, Gen rate: &3"
                  + String.format("%.2f", this.estimateChunkGenRate())));
      ...
      for (Entry<String, PerformanceCommand.RegionStats> entry : sortedRegions) {
         ...
         if (sender instanceof Player) {
            ...
            TextComponent regionCoords = new TextComponent(
                  ColorUtil.color("&3[" + worldName + "," + blockX + "," + blockZ + "]"));
            regionCoords.setHoverEvent(new HoverEvent(Action.SHOW_TEXT,
                  TextComponent.fromLegacyText(ColorUtil.color("&bClick to teleport to this region"))));
            regionCoords.setClickEvent(new ClickEvent(net.md_5.bungee.api.chat.ClickEvent.Action.RUN_COMMAND,
                  "/tp " + blockX + " 100 " + blockZ));
            message.addExtra(regionCoords);
            ((Player) sender).spigot().sendMessage(new BaseComponent[] { message });
         } else {
            sender.sendMessage(
                  ColorUtil.color("&b— Region around block &3[" + worldName + "," + blockX + "," + blockZ + "]"));
         }
         ...
         sender.sendMessage(
               ColorUtil.color(
                     "  &3"
                           ...
                           + " &bTPS"));
         sender.sendMessage(ColorUtil.color("  &bChunks: &3" + stats.chunks + " &b| Players: &3" + stats.players
               + " &b| Entities: &3" + stats.entities));
         count++;
      }
   ...
   private void showMemoryStats(CommandSender sender) {
      ...
      sender.sendMessage(
            ColorUtil.color("&7  • &6Used: &f" + heapUsed + " MB &7(" + String.format("%.1f", heapPercentage) + "%)"));
      ...
      if (nonHeapMax > 0L) {
         ...
         sender.sendMessage(ColorUtil
               .color("&7  • &6Max: &f" + nonHeapMax + " MB &7(" + String.format("%.1f", nonHeapPercentage) + "%)"));
      } else {
   ...
   private void showThreadStats(CommandSender sender) {
      ...
      try {
         ...
         sender.sendMessage(ColorUtil.color("&7  • &6Other Threads: &f"
               + (threadCount - asyncThreads - regionThreads - nettyThreads - workerThreads)));
      ...
      if (world == null) {
         ...
         if (!entityCounts.isEmpty()) {
            ...
            entityCounts.entrySet()
                  ...
                  .limit(10L)
                  .forEach(entry -> sender
                        .sendMessage(ColorUtil.color("&7  • &f" + entry.getKey() + ": &6" + entry.getValue())));
         }
   ...
   private void showTpsHistory(CommandSender sender) {
      ...
      synchronized (this.tpsHistory) {
         ...
         sender.sendMessage(ColorUtil
               .color("&6Min TPS: &f" + String.format("%.2f", min) + "  &6Max TPS: &f" + String.format("%.2f", max)));
   ...
   private Map<String, PerformanceCommand.RegionStats> getRegionStats() {
      ...
      for (World world : Bukkit.getWorlds()) {
         ...
         for (Entry<String, List<Chunk>> entry : regionChunks.entrySet()) {
            ...
            PerformanceCommand.RegionStats stats = new PerformanceCommand.RegionStats(worldName, regionX * 128,
                  regionZ * 128);
   ...
   private double getLowestRegionTps() {
      ...
      return regions.isEmpty() ? Bukkit.getTPS()[0]
            : regions.values().stream().mapToDouble(r -> r.tps).min().orElse(Bukkit.getTPS()[0]);
   }
   ...
   private String getLastGCTime() {
      try {
         GarbageCollectorMXBean[] garbageCollectors = ManagementFactory.getGarbageCollectorMXBeans()
