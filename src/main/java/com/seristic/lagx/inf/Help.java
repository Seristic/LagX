...
public class Help {
   ...
   public static void init() {
      commandsHelp.clear();
      commandsHelp
            .add(new Help.HoverCommand("/lagx help(h) <num>", "Lists all available commands.", "lagx.help", true));
      commandsHelp.add(new Help.HoverCommand("/lagx status(st)", "Quick server health overview with key metrics.",
            "lagx.status", true));
      commandsHelp.add(new Help.HoverCommand("/lagx master(m)", "Displays comprehensive server performance overview.",
            "lagx.master", true));
      commandsHelp
            .add(new Help.HoverCommand("/lagx tps", "Shows current server TPS with health status.", "lagx.tps", true));
      commandsHelp.add(new Help.HoverCommand("/lagx gc", "Runs garbage collection and shows memory improvement.",
            "lagx.gc", true));
      commandsHelp
            .add(new Help.HoverCommand("/lagx ram", "Displays detailed memory usage statistics.", "lagx.ram", true));
      commandsHelp.add(new Help.HoverCommand("/lagx protocol(pr) <options>",
            "Advanced protocol management and execution.", "lagx.protocol", true));
      commandsHelp.add(
            new Help.HoverCommand("/lagx clear(c)", "Removes various entities/items from worlds.", "lagx.clear", true));
      commandsHelp.add(
            new Help.HoverCommand("/lagx count(ct)", "Counts various entities/items in worlds.", "lagx.clear", true));
      commandsHelp.add(
            new Help.HoverCommand("/lagx clear area <c:chunks|b:blocks> [type]", "Clear entities in area around you.",
                  "lagx.clear", true));
      commandsHelp.add(new Help.HoverCommand("/lagx world(w) <world>",
            "Shows detailed statistics for a specific world.", "lagx.world", true));
      commandsHelp.add(new Help.HoverCommand("/lagx unload(u) <world>", "Unloads all chunks in the specified world.",
            "lagx.unload", true));
      ...
      commandsHelp
            .add(new Help.HoverCommand("/lagx info(i)", "Shows LagX version and information.", "lagx.help", true));
      commandsHelp.add(new Help.HoverCommand("/lagx ping(p) <player:none>", "Displays player connection latency.",
            "lagx.ping", true));
      commandsHelp.add(new Help.HoverCommand("/lagx preset [basic|advanced|custom]", "Switch entity limiter presets.",
            "lagx.entities", true));
      commandsHelp.add(new Help.HoverCommand("/lagx entities [status|reload|stats]", "Manage entity limiting system.",
            "lagx.entities", true));
      commandsHelp.add(
            new Help.HoverCommand("/lagx villagers [status|reload|optimize|stats]", "Optimize villager AI performance.",
                  "lagx.villagers", true));
      commandsHelp.add(
            new Help.HoverCommand(
                  "/lagx stacker(stack) [info|reload|stack <radius>]",
                  "Manage entity stacking system to reduce entity count.", "lagx.stacker", true));
      commandsHelp.add(new Help.HoverCommand("/lagx reload(rl)",
            "Reload plugin configuration without restarting the server.", "lagx.reload", true));
      commandsHelp.add(new Help.HoverCommand("/lagx warnings(warn) [status|on|off|toggle]",
            "Toggle or view protocol warnings.", "lagx.warn", true));
   }
   ...
   public static void send(Player p, int pageNum) {
      ...
      if (pageNum > maxPages) {
         ...
         Component header = ((TextComponent) ((TextComponent) ((TextComponent) ((TextComponent) ((TextComponent) ((TextComponent) Component
               .text(
   ...
   private static void sendPaginationButtons(Player p, int currentPage, int maxPages) {
      ...
      pagination = pagination
            .append(Component.text(" §8[§7" + currentPage + "§8/§7" + maxPages + "§8] ", NamedTextColor.GRAY));
         ...
         Component footer = ((TextComponent) Component.text("§7Tip: Use ", NamedTextColor.GRAY)
               .append(
                     Component.text("/lagx help <page>", NamedTextColor.YELLOW)
                           .hoverEvent(
                                 HoverEvent.showText(Component.text("Jump to specific page", NamedTextColor.GRAY)))))
   ...
   public static void sendMsg(Player p, String msg, boolean pre) {
      if (p == null) {
         LagX.instance.getLogger().info(msg.replaceAll("§[0-9a-fk-or]", ""));
      } else {
         if (pre) {
            msg = LagX.prefix + msg;
         }
   ...
   public static class HoverCommand {
      ...
      public Component toComponent() {
         Component hoverText = ((TextComponent) ((TextComponent) ((TextComponent) ((TextComponent) Component
               .text("Description: ", NamedTextColor.GOLD)
               ...
               .append(Component.text("\n")))
               .append(this.adminOnly ? Component.text("ADMIN ONLY", NamedTextColor.RED)
